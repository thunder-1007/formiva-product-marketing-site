...
import { getSession, saveSession, saveTelegramMessage, setCustomerTyping } from "./storage.js";

export default async function handler(request: ApiRequest, response: ApiResponse) {
  if (request.method !== "POST") return methodNotAllowed(response, ["POST"]);
  const body = bodyObject(request), sessionId = body?.sessionId, message = body?.message, messageId = body?.messageId, action = body?.action;
  if (!validSessionId(sessionId)) return response.status(400).json({ ok: false, error: "invalid_request" });
  try {
    if (action === "typing") {
      if (typeof body?.isTyping !== "boolean") return response.status(400).json({ ok: false, error: "invalid_request" });
    ...
    if (typeof message !== "string" || !message.trim() || message.length > MAX_MESSAGE_LENGTH || typeof messageId !== "string" || messageId.length > 160) return response.status(400).json({ ok: false, error: "invalid_request" });
