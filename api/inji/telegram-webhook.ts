...
export default async function handler(
  ...
  try {
    ...
    console.info("[inji] telegram message received", {
      ...
      replyToCaption: message.reply_to_message?.caption ?? null,
    });
    ...
    if (!session) {
      console.warn("[inji] resolved session does not exist", {
        sessionId: resolution.sessionId,
      });
      return response.json({ ok: true });
    }
    ...
    if (session.status !== "active") {
      console.warn("[inji] resolved session is not active", {
        sessionId: session.sessionId,
    ...
    if (session.messages.some((item) => item.id === humanMessageId)) {
      return response.json({ ok: true });
    }
